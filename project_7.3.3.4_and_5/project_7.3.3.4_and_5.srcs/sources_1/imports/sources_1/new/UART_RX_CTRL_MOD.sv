...
module UART_RX_CTRL_MOD(
    ...
    output logic [1:0] ALU_ctrl,display_ctrl,
    output logic trigger_TX_result,
    output logic [3:0] state_peek
    );
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
             ...
             case (state)
                ...
                Store_OP1_MSB: begin operando1[15:8] <= rx_data;
                                     display_ctrl    <= 2'b01; end
                                     
                Store_OP2_LSB:       operando2[7:0] <= rx_data;
                
                Store_OP2_MSB: begin operando2[15:8] <= rx_data;
                                     display_ctrl    <= 2'b10; end 
                
                Store_CMD    : begin ALU_ctrl       <= rx_data[1:0];
                                     display_ctrl   <= 2'b11; end
                endcase end end
