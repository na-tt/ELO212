...
module debug_display_calc_FSM(
    ...
    logic CLK1HZ;
    clock_divider #(50000000) slowclk(.clk_in(CLK100MHZ),.reset(0),.clk_out(CLK1HZ)); // generates 1hz clock
    ...
    basic_calc_FSM #(16) FSM(.clk(CLK1HZ),.reset(CPU_reset),.next(1),.bits(switches),.show(show),.result(result),.FSM_state({LED1,LED0}));
